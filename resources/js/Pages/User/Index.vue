...
const deactivateButton = (user) => {
    form.is_activate = user.is_active;

    Swal.fire({
        title: `Are you sure you want to ${
            form.is_activate ? "Deactivate" : "Activate"
        } this account?`,
        text: `You ${
            form.is_activate ? "wont" : "will"
        ...
        confirmButtonText: ` ${
            form.is_activate ? "Yes, deactivate it," : "Yes activate it"
        } this account?`,
        showClass: {
        ...
        if (result.isConfirmed) {
            form.put("/user/deactivate/" + user.id, {
                onError: (error) => {
                    console.log("Error" + error);
                },
                onSuccess: () => {
                    console.log("Success");
                    toast.success("User successfully deactivate", {
                        timeout: 2000,
                    });
                },
            });
                            ...
                            >
                                ...
                                >
                                    <div class="flex items-center space-x-4">
                                        <button
                                            @click="editButton(user)"
                                            type="button"
                                            aria-controls="drawer-update-user"
                                            class="py-2 px-3 flex items-center text-sm font-medium text-center text-gray-900 focus:outline-none bg-white rounded-lg border border-cyan-500 hover:bg-cyan-600 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                                        >
                                            <svg
                                                class="w-4 h-4 mr-2 text-white-800 dark:text-white"
                                                aria-hidden="true"
                                                xmlns="http://www.w3.org/2000/svg"
                                                width="24"
                                                height="24"
                                        ...
                                        <button
                                            type="button"
                                            @click="deactivateButton(user)"
                                            aria-controls="drawer-read-product-advanced"
                                            class="w-36 py-2 px-3 flex items-center text-sm font-medium text-center text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                                        >
                                            <svg
